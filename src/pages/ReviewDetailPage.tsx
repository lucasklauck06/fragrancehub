  ...
  if (!review) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center">
        <p className="text-xl font-semibold text-foreground mb-4">Resenha não encontrada</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="overflow-hidden border border-border bg-background/95 shadow-sm rounded-xl">
            ...
            <div className="bg-muted/50/50 border-b border-border p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <CardContent className="p-6">
              {isEditing ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground font-semibold">Nota:</span>
                  ...
                  <textarea
                    ...
                    className="w-full text-sm border border-border rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-teal-600/50"
                    rows={4}
                    placeholder="Sua avaliação detalhada..."
                  />

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-muted/50 p-4 rounded-xl border border-border">
                  ...
                  <div className="flex justify-end gap-2 pt-4">
                    ...
                    <Button size="sm" className="bg-primary hover:bg-teal-700" onClick={handleSave} disabled={saving}>Salvar Alterações</Button>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="mb-8 relative">
                    ...
                    <p className="text-muted-foreground leading-relaxed text-base md:text-lg whitespace-pre-line relative z-10 pl-4">
                      {review.comment}
                    </p>
              ...
              {/* Technical Metrics */}
                <div className="mt-8 pt-6 border-t border-border">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-4">Métricas Técnicas Avaliadas</h3>
                  <div className="flex flex-wrap gap-4">
                    {review.longevidade != null ? (
                      <div className="flex bg-teal-50 px-4 py-2 rounded-lg border border-teal-100 items-center gap-2 flex-1 min-w-[140px]">
                        ...
                        <div>
                          <p className="text-[10px] text-primary/80 uppercase font-bold tracking-wider">Fixação</p>
                          <p className="font-bold text-teal-800">{review.longevidade} / 5</p>
                        </div>
                      </div>
                    ) : (
                      <div className="flex bg-teal-50 px-4 py-2 rounded-lg border border-teal-100 items-center gap-2 flex-1 min-w-[140px]">
                        <p className="text-[10px] text-primary/80 uppercase font-bold tracking-wider">Sem fixação</p>
                      </div>
                    )}
                    {review.rastro != null ? (
        ...
        <div className="space-y-6">
          {/* Perfume Card Info */}
          <Card className="border border-border bg-background/95 shadow-sm rounded-xl overflow-hidden cursor-pointer hover:border-teal-200 transition-colors" onClick={() => navigate(`/perfume/${review.perfumeId}`)}>
            <div className="bg-white h-48 w-full flex items-center justify-center p-4 border-b border-border">
              <img
                ...
                className="h-full object-contain mix-blend-multiply bg-white"
              />
            </div>
            <CardContent className="p-4">
              <div className="text-center">
                <p className="text-xs font-bold text-primary uppercase tracking-wider mb-1">{review.perfumeBrand}</p>
                <h3 className="font-bold text-foreground text-lg leading-tight mb-2">{review.perfumeName}</h3>
                <div className="flex justify-center gap-2 mb-3">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full text-white ${review.perfumeGender === "Masculino" ? "bg-blue-500" : review.perfumeGender === "Feminino" ? "bg-pink-500" : "bg-primary"
                    }`}>
