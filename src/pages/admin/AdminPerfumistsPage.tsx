...
export default function AdminPerfumistsPage() {
  ...
  const fetchPerfumists = async () => {
    try {
      const res = await fetch("http://localhost:3000/api/perfumists");
      if (res.ok) {
        const data = await res.json();
        setPerfumists(data);
      } else {
        toast.error("Erro ao carregar os perfumistas do servidor.");
      }
    } catch (error) {
      console.error(error);
      toast.error("Falha na conexão com o servidor. Tente novamente.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredAndSortedPerfumists = useMemo(() => {
    let result = [...perfumists];
    ...
    return result;
  }, [perfumists, searchQuery, sortField, sortDirection]);

  const handleDelete = async () => {
    if (!deleteId) return;

    try {
      const perfumist = perfumists.find((p) => p.id === deleteId);
      const token = localStorage.getItem("token");

      const res = await fetch(`http://localhost:3000/api/perfumists/${deleteId}`, {
        method: "DELETE",
        headers: {
      ...
      if (res.ok) {
        setPerfumists(perfumists.filter((p) => p.id !== deleteId));
        setDeleteId(null);
        toast.success(`Perfumista "${perfumist?.name}" excluído com sucesso!`);
      } else {
        toast.error("Erro ao excluir o perfumista. Verifique suas permissões.");
      }
    } catch (error) {
  ...
  const perfumistToDelete = perfumists.find((p) => p.id === deleteId);
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              onClick={() => navigate("/admin/perfumistas/novo")}
              className="gap-2 bg-green-600 hover:bg-green-700"
            >
              <Plus size={16} /> Novo Perfumista
            </Button>
          </div>
        </CardHeader>
        <CardContent>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => navigate(`/admin/perfumistas/${perfumist.id}`)}
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => setDeleteId(perfumist.id)}
